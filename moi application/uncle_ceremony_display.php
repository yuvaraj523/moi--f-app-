...
?><?php
include('db.php');
$sql = "SELECT * FROM uncle_ceremony ORDER BY date DESC";
...
<head>
    ...
    <title>List  Maternal Moi Dashboard</title>
...
?>
<div class="container">
    <form method="POST" action="uncle_delete_record.php">
                ...
                <?php while($row = $result->fetch_assoc()): ?>
                    ...
                    <td><?= htmlspecialchars($sno++); ?></td>
                     <td><?php echo htmlspecialchars($row['date']); ?></td>
                        ...
                        <td class="actions">
                            <a href="uncle_ceremony_edit.php?id=<?php echo htmlspecialchars($row['id']); ?>" title="Edit"><i class="fas fa-edit"></i></a>
                            <a href="uncle_ceremony_view.php?id=<?php echo htmlspecialchars($row['id']); ?>" title="View"><i class="fas fa-eye"></i></a>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function generatePDF() {
       ...
       // Calculate column widths based on content
        ...
        let currentY = 23;
        const cellPadding = 3;
        const columnWidths = headers.map(header => Math.max(header.length * 3.7, 23));
        ...
        // Save the PDF
        doc.save('maternal_uncle_ceremony_details.pdf');
    }
...
function confirmDelete(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            window.location.href = `uncle_ceremony_delete.php?id=${id}`;
        }
    });
}
