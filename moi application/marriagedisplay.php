...
$sql = "SELECT * FROM wedding ORDER BY date DESC";
...
<head>
    ...
    <title>list Marriage Detail Dashboard</title>
...
?>
<div class="container">
    <form method="POST" action="marriage_delete_record.php">
        ...
        <tbody>
            ...
            <?php if ($result && $result->num_rows > 0): ?>
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                   ...
                   <td><?php echo htmlspecialchars($row['swaminame']); ?></td>
                        <td><?php echo htmlspecialchars($row['the_groom']); ?></td>
                        <td><?php echo htmlspecialchars($row['the_bride']); ?></td>
                        <td><?php echo htmlspecialchars($row['date']); ?></td>
                        <td><?php echo htmlspecialchars($row['mahal']); ?></td>
                        <td><?php echo htmlspecialchars($row['place']); ?></td>
                        <td><?php echo htmlspecialchars($row['timing']); ?></td>
                        <td class="actions">
                            <a href="marriage_edit.php?id=<?php echo htmlspecialchars($row['id']); ?>" title="Edit"><i class="fas fa-edit"></i></a>
                            <a href="marriage_view.php?id=<?php echo htmlspecialchars($row['id']); ?>" title="View"><i class="fas fa-eye"></i></a>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function generatePDF() {
        ...
        doc.setFontSize(12);
        const headerX = 14;
        let currentY = 30; // Start position for the first header
        const cellPadding = 3;
        const columnWidths = headers.map(header => Math.max(header.length * 6.3, 30)); // Minimum width
        ...
        rows.forEach(row => {
            currentX = headerX; // Reset x for each row
            row.forEach((cell, i) => {
            ...
            if (currentY > 260) { // Adjust if needed
                ...
                currentX = headerX; // Reset x for headers
                headers.forEach((header, i) => {
                    doc.text(header, currentX, currentY);
                    currentX += columnWidths[i];
                });
        ...
        // Save the PDF
        doc.save('maternal_uncle_details.pdf');
    }
...
function confirmDelete(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            window.location.href = `marriage_delete.php?id=${id}`;
        }
    });
}
